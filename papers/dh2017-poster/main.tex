...
We use text reuse detection to identify passages from George Eliot's novel \emph{Middlemarch} that are present in a corpus of over 4,000 critical articles obtained from JSTOR that contain the word ``Middlemarch''. We attempt to answer questions like: 

\begin{itemize}
\item What passages of the novel are critics most interested in quoting?  
\item How do these critical favorites change over time? 
...
We use a custom-built text reuse detection algorithm, (``text-matcher'' on the Python package archive PyPi), that uses a techniques of approximate text matching in a two-pass process to identify similar passages between two texts. The first pass uses sequence matching to find a single matching trigram between the two texts. The algorithm then compares ratios of Levenstein distances of boundary words to determine whether the match can be expanded. Contiguous matches are then healed, and longest possible matches returned.
